...
const sample = {
  workers: [
    { id: "W-1042", name: "Maya Patel", role: "Senior Finance Manager", department: "Finance", manager: "Daniel Kim", performance: 92, potential: 88, goals: 94, skills: 91, experience: 84, qualifications: 100, readiness: "Ready now", aspiration: "Interested", interestDate: "Aug 12, 2026", cell: "future-leader", nominations: 2 },
    { id: "W-1088", name: "Jordan Lee", role: "Finance Transformation Lead", department: "Finance", manager: "Daniel Kim", performance: 86, potential: 81, goals: 89, skills: 87, experience: 78, qualifications: 92, readiness: "Ready < 1 year", aspiration: "Open to discuss", interestDate: "Sep 4, 2026", cell: "future-leader", nominations: 1 },
    ...
    { id: "W-1196", name: "Aiden Brooks", role: "FP&A Manager", department: "Finance", manager: "Daniel Kim", performance: 77, potential: 91, goals: 74, skills: 79, experience: 70, qualifications: 88, readiness: "Ready 1-2 years", aspiration: "Not confirmed", interestDate: "Manager recorded Sep 18, 2026", cell: "growth-candidate", nominations: 0 },
  ...
  nominations: [
    { worker: "Maya Patel", id: "W-1042", readiness: "Ready now", rank: 1, fit: 91, interest: "Employee confirmed", status: "Approved", source: "Manual", evidence: "Current" },
    { worker: "Elena Garcia", id: "W-1134", readiness: "Ready now", rank: 2, fit: 88, interest: "Open to discuss", status: "Approved", source: "Recommendation", evidence: "Current" },
    { worker: "Jordan Lee", id: "W-1088", readiness: "Ready < 1 year", rank: 3, fit: 85, interest: "Employee confirmed", status: "Under review", source: "Recommendation", evidence: "1 warning" }
  ],
  developmentActions: [
    { worker: "Maya Patel", objective: "Board-level financial communication", type: "Coaching", owner: "Daniel Kim", target: "Dec 15, 2026", status: "In progress" },
    { worker: "Jordan Lee", objective: "Treasury risk exposure", type: "Stretch assignment", owner: "Elena Garcia", target: "Mar 31, 2027", status: "Planned" },
    { worker: "Aiden Brooks", objective: "Enterprise planning transformation", type: "Project", owner: "Nora Ali", target: "Jun 30, 2027", status: "Draft" }
  ],
  agents: [
    ["Critical Role Discovery", "Detects vacancy impact, scarce skills and weak coverage", "HR approves criticality"],
    ["Candidate Discovery", "Finds permitted candidates and explains fit", "Planner creates nominations"],
    ["Evidence", "Builds a sourced readiness summary", "User validates sources"],
    ["Succession Planning", "Drafts plans, gaps and follow-up actions", "Plan owner approves"],
    ["Calibration Preparation", "Finds missing evidence, outliers and inconsistencies", "Facilitator reviews"],
    ["Calibration Meeting Copilot", "Summarizes discussion and proposed decisions", "Participants confirm"],
    ["Coverage Recovery", "Suggests alternatives for uncovered targets", "Plan owner approves changes"],
    ["Development Action", "Turns verified gaps into Draft actions", "Employee and manager agree"],
    ["Scenario Planning", "Simulates moves and downstream vacancies", "Executive chooses scenario"],
    ["Approval Brief", "Summarizes evidence, changes, risks and exceptions", "Approver decides"],
    ["Engagement", "Sends secure personalized nudges", "Administrator controls cadence"],
    ["Fairness Monitor", "Flags disparity, proxy risk and unequal opportunity", "HR/ethics investigates"],
    ["Configuration Validator", "Tests setup, permissions, dates and dependencies", "Administrator publishes"],
    ["Outcome Learning", "Measures readiness, retention and placement outcomes", "Governed updates only"]
  ],
  distribution: [
  ...
  setup: [
    ...
    { route: "skills", title: "Dynamics HR skills and competencies", detail: "Reuse source masters, proficiency mappings and evidence policy", status: "Configured", marker: "reused" },
...
const routeMeta = {
  ...
  skills: ["Skills and competency reuse", "Setup / Succession management", "Setup"],
...
const configPages = {
  ...
  skills: {
    marker: "reused", summary: "Existing Dynamics HR skills, proficiency levels, Job mappings and worker competency records remain authoritative; succession stores only governed mappings and evidence snapshots.",
    fields: [
      ["Skill master", "Dynamics HR Skills", "No parallel succession catalogue"], ["Job requirement source", "Job skills and Skill mapping", "Position overrides display their source"],
      ["Proficiency normalization", "Existing level to 0-100", "Versioned mapping used only for assessment"], ["Mandatory skill behavior", "Exclude with explanation", "Recommendation profile controls policy"],
      ["Employee-declared skills", "Show as unverified", "Cannot become recorded fact without confirmation"], ["Evidence stale after", "365 days", "Creates refresh activity"],
      ["Qualification expiry", "Enforce mandatory", "Expired evidence cannot satisfy eligibility"], ["Future inference", "Suggestion only", "Worker or authorized reviewer confirms"]
    ],
    effect: "Source IDs, proficiency, verification, mapping version and as-of date are preserved in Candidate Assessment Evidence. Approved snapshots never change when a source mapping is revised."
  },
  automation: {
...
const navGroups = [
  ...
  ["Configuration", [["setup", "Setup checklist"], ["rating", "Rating models"], ["parameters", "Parameters"], ["skills", "Skills and competencies"], ["automation", "Automation and AI"]]],
...
function planView() {
  return `${pageHeader(`<button class="btn" data-action="compare">Compare candidates</button><button class="btn" data-action="nominate">Nominate successor</button><button class="btn primary" data-action="submit-plan">Submit plan</button>`)}
    <div class="tabs"><button class="tab active">Candidates</button><button class="tab">Development</button><button class="tab">Coverage</button><button class="tab">Evidence</button><button class="tab">Activity</button><button class="tab">Workflow</button><button class="tab">History</button></div>
    ...
    <section class="section"><div class="section-title"><h2>Successor nominations</h2><div class="legend">${marker("new")} ${marker("ai")}</div></div><div class="data-table-wrap"><table class="data-table"><thead><tr><th>Rank</th><th>Candidate</th><th>Readiness</th><th>Role fit</th><th>Interest</th><th>Status</th><th>Source</th><th>Evidence</th><th></th></tr></thead><tbody>${sample.nominations.map(n => `<tr><td>${n.rank}</td><td><button class="link-button" data-action="worker" data-worker="${n.id}">${n.worker}<br><span class="small muted">${n.id}</span></button></td><td>${n.readiness}</td><td><strong>${n.fit}</strong>/100</td><td>${status(n.interest, n.interest === "Employee confirmed" ? "good" : "info")}</td><td>${status(n.status, n.status === "Approved" ? "good" : "info")}</td><td>${n.source === "Recommendation" ? marker("ai", "AI proposed") : "Manual"}</td><td>${n.evidence === "Current" ? status("Current", "good") : status(n.evidence, "warn")}</td><td><button class="btn subtle" data-action="worker" data-worker="${n.id}">Review</button></td></tr>`).join("")}</tbody></table></div></section>
    <section class="section"><div class="section-title"><div><h2>Development actions</h2><span class="small muted">Lightweight follow-through linked to readiness gaps; Dynamics HR modules continue to own Courses, Goals, Skills and worker actions.</span></div><button class="btn" data-action="development-action">Add action</button></div><div class="data-table-wrap"><table class="data-table"><thead><tr><th>Candidate</th><th>Objective</th><th>Type</th><th>Owner</th><th>Target date</th><th>Status</th></tr></thead><tbody>${sample.developmentActions.map(action => `<tr><td>${action.worker}</td><td>${action.objective}</td><td>${action.type}</td><td>${action.owner}</td><td>${action.target}</td><td>${status(action.status, action.status === "In progress" ? "good" : action.status === "Draft" ? "warn" : "info")}</td></tr>`).join("")}</tbody></table></div></section>
...
function configView(route) {
  ...
  return `${pageHeader(actions)}
    ...
    ${route === "automation" ? `<section class="section"><div class="section-title"><div><h2>AI and agent catalogue</h2><span class="small muted">Agents summarize, recommend or prepare Draft work; consequential talent decisions stay with people.</span></div>${marker("required", "Human control")}</div><div class="grid cols-2">${sample.agents.map(([name, purpose, control]) => `<article class="card flat"><div class="card-head"><h3>${name} Agent</h3>${marker("ai")}</div><p>${purpose}</p><div class="small"><strong>Human control:</strong> ${control}</div></article>`).join("")}</div><div class="ai-callout section"><strong>Required output contract</strong><p>Every proposal shows evidence and source date, confidence, missing information, rationale, alternatives, fairness checks, human approval, immutable audit metadata, and generated-versus-recorded status.</p></div></section>` : ""}`;
}
...
function modalView() {
  ...
  const content = {
    ...
    nominate: ["Nominate successor", `<div class="form-grid"><div class="field full"><label>Worker<span class="required-star">Required</span></label><select><option>Aiden Brooks | W-1196</option><option>Fatima Rahman | W-1301</option></select></div><div class="field"><label>Readiness</label><select><option>Ready 1-2 years</option></select></div><div class="field"><label>Rank</label><input value="4"></div><div class="field"><label>Candidate interest</label><select><option>Not confirmed</option></select><div class="field-help">Manager recorded Sep 18; employee confirmation pending</div></div><div class="field"><label>Interest valid until</label><input value="09/18/2027"><div class="field-help">AI cannot infer interest or mobility constraints</div></div><div class="field"><label>Estimated ready date</label><input value="09/01/2028"></div><div class="field full"><label>Nomination rationale</label><textarea>Strong potential and transformation leadership; requires target-specific treasury exposure.</textarea></div></div>`],
    "development-action": ["Add development action", `<div class="form-grid"><div class="field"><label>Candidate<span class="required-star">Required</span></label><select><option>Aiden Brooks | W-1196</option></select></div><div class="field"><label>Action type<span class="required-star">Required</span></label><select><option>Project / stretch assignment</option><option>Learning</option><option>Goal</option><option>Coaching</option></select></div><div class="field full"><label>Gap or objective<span class="required-star">Required</span></label><input value="Enterprise planning transformation experience"></div><div class="field"><label>Owner<span class="required-star">Required</span></label><input value="Nora Ali"></div><div class="field"><label>Target date<span class="required-star">Required</span></label><input value="06/30/2027"></div><div class="field full"><div class="ai-callout"><strong>${marker("ai")} Development Action Agent</strong><p>Drafted from verified skill and experience gaps. Employee and manager agreement is required before Planned status; completion cannot change readiness automatically.</p></div></div></div>`],
...
function workerDrawer(workerId) {
  ...
  const workerActions = sample.developmentActions.filter(action => action.worker === worker.name);
  return `<div class="drawer" role="complementary" aria-label="Worker evidence"><div class="drawer-head"><div><h2>${worker.name}</h2><div class="small muted">${worker.id} | ${worker.role}</div></div><button class="close-button" data-action="close-drawer" aria-label="Close">&times;</button></div><div class="drawer-body"><div class="legend">${marker("reused", "Worker")} ${marker("enhanced", "Talent summary")}</div><section class="section card flat"><h3>Employee aspiration</h3><p><strong>${worker.aspiration || "Not recorded"}</strong></p><div class="small muted">${worker.interestDate || "Confirmation required before selection"} | Visibility: Employee, manager and authorized HR</div></section><section class="section"><h3>Assessment evidence</h3>${[["Performance",worker.performance],["Potential",worker.potential],["Skills",worker.skills],["Weighted goals",worker.goals],["Experience",worker.experience],["Qualifications",worker.qualifications]].map(([label,value]) => `<div class="score-row"><span>${label}</span><div class="score-track"><div class="score-fill" style="width:${value}%"></div></div><strong>${value}</strong></div>`).join("")}<div class="score-row"><strong>Role fit</strong><div class="score-track"><div class="score-fill" style="width:${fit}%"></div></div><strong>${fit}</strong></div></section><section class="section ai-callout"><strong>${marker("ai")} Grounded summary</strong><p>${worker.name} meets all mandatory qualifications. Strongest evidence is weighted goals (${worker.goals}) and skill fit (${worker.skills}). Readiness is ${worker.readiness}. This summary cites approved FY2026 evidence and does not change a rating.</p></section><section class="section"><h3>Dynamics HR skill evidence</h3><ul class="evidence-list"><li>Source: Job skills and worker Skill competency</li><li>Proficiency mapping: SKILL-MAP-FIN-V2</li><li>Verification: Manager assessed | As of Sep 12, 2026</li><li>Inferred skills: none used as recorded facts</li></ul></section><section class="section"><h3>Development actions</h3>${workerActions.length ? workerActions.map(action => `<article class="card flat"><strong>${action.objective}</strong><p class="small muted">${action.type} | Owner: ${action.owner} | Due ${action.target}</p>${status(action.status, "info")}</article>`).join("") : `<p class="small muted">No active development action.</p>`}</section><section class="section"><h3>Source records</h3><ul class="evidence-list"><li>Performance review PR-FY26-${worker.id}</li><li>Weighted goals result WG-FY26-${worker.id}</li><li>Calibration result CAL-2026-017-${worker.id}</li><li>Worker skills snapshot SK-${worker.id}-09</li></ul></section><div class="card-footer"><button class="btn" data-action="development-action">Add development action</button><button class="btn primary" data-action="nominate">Nominate as successor</button></div></div></div>`;
}
...
function aiDrawer(kind) {
  ...
  return `<div class="drawer" role="complementary" aria-label="AI suggestions"><div class="drawer-head"><div><h2>${title}</h2><div class="small muted">${subtitle}</div></div><button class="close-button" data-action="close-drawer" aria-label="Close">&times;</button></div><div class="drawer-body"><div class="legend">${marker("ai")} ${marker("required", "Human review")}</div><div class="ai-callout section"><strong>Grounded outcome</strong><p>Generated from records the signed-in user can access. Suggestions are not decisions and are retained with model, prompt and source versions.</p></div><section class="section"><h3>Proposed work</h3>${items.map((item,index) => `<article class="card flat" style="margin-bottom:8px"><strong>${index + 1}. ${item}</strong><p class="small muted">Confidence ${88-index*5}% | Evidence as of Sep 24, 2026</p><p class="small">Missing information: ${index === 1 ? "employee confirmation" : "none material"} | Alternative available | Fairness check passed</p><div class="command-bar"><button class="btn">View sources</button><button class="btn primary" data-action="accept-ai">Prepare draft</button></div></article>`).join("")}</section><section class="section"><h3>Operational reduction</h3><div class="grid cols-2"><div class="card flat"><div class="small muted">Estimated page transitions</div><div class="kpi-value">-19</div></div><div class="card flat"><div class="small muted">Fields prepared</div><div class="kpi-value">24</div></div></div></section><section class="section"><h3>Sources and controls</h3><ul class="evidence-list"><li>Active configuration versions and source dates</li><li>Approved assessments and calibration results</li><li>Worker, Job and Position records</li><li>Generated suggestion; not a recorded fact</li><li>Immutable model, prompt and decision audit</li></ul></section></div></div>`;
}
...
function renderMain() {
  switch (state.route) {
    ...
    case "rating": case "parameters": case "criticality": case "distribution": case "matrix-config": case "calibration-config": case "skills": case "recommendation": case "automation": return configView(state.route);
...
function sectionRoute(section, item) {
  ...
  if (/employee aspiration|candidate interest|development action/.test(text)) return "plan";
  if (/skill|competenc/.test(text)) return "skills";
...
document.addEventListener("click", event => {
  ...
  if (["create-plan", "nominate", "development-action", "compare", "impact-preview", "assess-criticality"].includes(action)) { state.modal = action; render(); return; }
